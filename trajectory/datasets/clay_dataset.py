...
# dataset for the clay demonstrations

"""
Need a dictionary with states, rewards and actions sub-divided into each trajectory
    - trajectory length varies

For training - there is a fixed trajectory window length
    - need to record trajectory length
    - need to have a sliding window of seq_len
    - calculate reward for this sliding window, where we set the goal to be the final state in this window
...
Segmented Trajectories:
    ...
    trajectories[episode_num]["target_shape"]
    the goal is just the final state in states
